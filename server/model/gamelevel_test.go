...
func TestPourValid(t *testing.T) {
	...
	_, err := pour(level.GetStartState(), 0, 1)
	if err != nil {
		t.Fatalf(`Unable to pour. Level: %v \n Error: %v`, &level, err)
	}
	assert.Equal(t, []pb.Color{pb.Color_RED, pb.Color_GREEN, pb.Color_GREEN}, level.GetStartState().GetTubes()[1].GetColors(), "Tube colors should match")
}
...
func TestGamePlay(t *testing.T) {
	...
	for _, move := range solve {
		_, err := pour(level.GetStartState(), move[0], move[1])
		if err != nil {
